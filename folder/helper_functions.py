    ...
    with codecs.open(path_out, 'w', 'UTF-8') as f:
        ...
        while True:
            ...
            if end <= start:
                break
            if verbose:
                sys.stdout.write(' lines {0} => {1}\r'.format(start, end))
                sys.stdout.flush()
            
            # Create feed_dict using ref() for each tensor
            feed_dict = {
                x_ph: seqs[start:end, :],
            ...
            print(f"predictions_ph:{predictions_ph.ref}")
            
            p = session.run(predictions_ph, feed_dict=feed_dict)
        ...
        for i in range(P.shape[0]):
            pred_sym_seq = [loader.deenc_output.get_sym(x) for x in P[i, :lens[i]]]
            pred_str = ''
